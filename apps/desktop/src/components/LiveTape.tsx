...
import { EventSummary, recentEvents } from "../lib/tauri-bridge";
...
export default function LiveTape({ pollMs = 5000, limit = 50 }: Props) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const next = await recentEvents(limit);
        if (!cancelled) {
          setEvents(next);
          setError(null);
        }
        ...
        {events.map((event) => (
          ...
          >
            <div className="flex items-baseline justify-between gap-3">
              <span className="font-mono uppercase text-neutral-400">{event.event_type}</span>
