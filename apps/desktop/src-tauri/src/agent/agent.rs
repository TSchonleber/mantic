...
impl Agent {
    ...
    pub async fn handle_signal(&self, signal: Signal) -> Result<()> {
        ...
        let chain = match &outcome {
            ...
            | DecisionOutcome::Skip {
                reasoning_chain, ..
            } => reasoning_chain.clone(),
        };
        let _ = self.brainctl.event_add("decision", &chain, Some(0.7)).await;
...
mod tests {
    ...
    async fn dispatch_when_idle_errors() {
        let agent =
            agent_with_reply(r#"{"action":"skip","reasoning_summary":"x","reasoning_chain":"y"}"#);
        let err = agent.handle_signal(Signal::new_test("BONK")).await.unwrap_err();
        assert!(matches!(
            err,
            crate::error::AppError::AgentInvalidState(_)
        ));
    }
