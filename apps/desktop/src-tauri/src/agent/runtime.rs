...
impl AgentRuntime {
    ...
    pub fn brainctl(&self) -> Arc<BrainctlClient> {
        self.brainctl.clone()
    }

    pub fn llm_keys(&self) -> Arc<LlmKeyStore> {
        self.llm_keys.clone()
    }
    ...
    pub async fn spawn(&self, config: AgentConfig) -> Result<AgentSummary> {
        ...
        self.spawn_agent(config, llm).await
    }
    ...
    async fn spawn_agent(
        ...
        self.agents
            .lock()
            .await
            .insert(config.id.clone(), agent.clone());
        Ok(AgentSummary {
            id: config.id.clone(),
    ...
    pub async fn get(&self, id: &str) -> Result<AgentDetails> {
        ...
        let a = agents
            .get(id)
            .cloned()
            .ok_or_else(|| AppError::AgentNotFound(id.into()))?;
        drop(agents);
    ...
    pub async fn arm(&self, id: &str) -> Result<AgentSummary> {
        ...
        if !cur.can_arm() {
            return Err(AppError::AgentInvalidState(format!(
                "cannot arm from {cur:?}"
            )));
        }
    ...
    #[cfg(any(test, feature = "test-helpers"))]
    pub fn executor(&self) -> Arc<PaperExecutor> {
        self.executor.clone()
    }
}
