...
for branch in "${BRANCHES[@]}"; do
    ...
    # Copy the bundled sidecar binary into the worktree so tauri.conf.json's externalBin
    # resolves at cargo-test time. The binary itself is gitignored.
    if [ -d "$REPO_ROOT/apps/desktop/src-tauri/binaries" ]; then
        mkdir -p "$wt/apps/desktop/src-tauri/binaries"
        cp -p "$REPO_ROOT/apps/desktop/src-tauri/binaries/"* \
              "$wt/apps/desktop/src-tauri/binaries/" 2>/dev/null || true
    fi

    # Pre-stage the wallet-bridge dist so build.rs's copy step doesn't warn.
    if [ -d "$REPO_ROOT/apps/wallet-bridge/dist" ]; then
        mkdir -p "$wt/apps/wallet-bridge/dist"
        cp -p "$REPO_ROOT/apps/wallet-bridge/dist/"* \
              "$wt/apps/wallet-bridge/dist/" 2>/dev/null || true
    fi
